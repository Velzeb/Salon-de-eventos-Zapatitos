        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-10 space-y-10">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Total Evento</span>
              <div className="text-xl font-black text-bg-dark mt-1">${totalPrice.toLocaleString()}</div>
            </div>
            <div className="bg-primary/5 rounded-2xl p-6 border border-primary/10">
              <span className="text-[9px] font-bold text-primary uppercase tracking-widest">Saldo Pendiente</span>
              <div className="text-xl font-black text-primary mt-1">${saldoPendiente.toLocaleString()}</div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-black text-bg-dark uppercase tracking-[0.2em]">Registro de Abonos</h3>
              {!isAdding && saldoPendiente > 0 && (
                <button 
                  className="flex items-center gap-2 bg-bg-dark text-white px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-primary transition-all shadow-lg active:scale-95"
            ...
            {isAdding && (
              <div className="bg-white border border-slate-100 rounded-[2.5rem] p-8 shadow-xl animate-in zoom-in-95 duration-300">
                <div className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Monto del Pago</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Método de Selección</label>
                    <select 
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Referencia / Notas</label>
                    <input 
                      type="text"
                      className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 outline-none focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all font-bold text-sm"
                  ...
                  <div className="flex gap-4 pt-4">
                    <button 
                      className="flex-1 px-8 py-4 rounded-2xl border border-slate-100 text-[10px] font-bold uppercase tracking-widest text-slate-400 hover:bg-slate-50 transition-all active:scale-95"
                      onClick={() => setIsAdding(false)}
                    >
                      Cancelar
                    </button>
                    <button 
                      className="flex-[2] bg-primary text-white py-4 rounded-2xl shadow-lg shadow-primary/20 font-bold uppercase tracking-widest text-xs hover:scale-[1.02] active:scale-[0.98] transition-all"
          ...
          <div className="space-y-6 pb-10">
            <h3 className="text-xs font-black text-bg-dark uppercase tracking-[0.2em]">Historial de Transacciones</h3>
            
            <div className="space-y-4">
              {pagos.map(p => (
                <div key={p.id} className="flex flex-col p-6 bg-white border border-slate-100 rounded-[1.5rem] hover:bg-slate-50 transition-all hover:translate-x-1 gap-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[8px] px-2 py-0.5 rounded-full uppercase font-bold ${p.estado === 'Pendiente' ? 'bg-amber-100 text-amber-700' : p.estado === 'Rechazado' ? 'bg-rose-100 text-rose-700' : 'bg-emerald-100 text-emerald-700'}`}>
                            {p.estado}
                          </span>
                        </div>
                        <span className="text-sm font-bold text-bg-dark uppercase tracking-tight mt-1">{p.metodoPago || 'Efectivo/Transferencia'}</span>
                        {p.referencia && <span className="text-[10px] font-medium text-slate-400 mt-0.5">{p.referencia}</span>}
                      </div>
                    </div>
                    <div className="text-lg font-black text-bg-dark group-hover:text-primary transition-colors">
                    ...
                    <div className="pt-4 mt-2 border-t border-slate-100 flex flex-col gap-4">
                      <div className="flex gap-4 items-start">
                        <div className="flex-1 space-y-2">
                          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Comprobante Adjunto</p>
                        ...
                        <div className="flex flex-col gap-2 shrink-0">
                          <button 
                            ...
                            disabled={loading}
                            className="bg-emerald-500 text-white text-xs font-bold uppercase tracking-widest px-4 py-2 rounded-lg hover:bg-emerald-600 transition-colors"
                          >
                            Aprobar Pago
                          </button>
                          <button 
                            ...
                            disabled={loading}
                            className="bg-rose-50 text-rose-600 text-xs font-bold uppercase tracking-widest px-4 py-2 rounded-lg hover:bg-rose-100 transition-colors border border-rose-200"
                          >
                            Rechazar
                          </button>
                        </div>
              ...
              {pagos.length === 0 && (
                <div className="py-20 flex flex-col items-center justify-center space-y-4 bg-slate-50 rounded-[2.5rem] border border-slate-100 border-dashed">
                  <Receipt size={48} className="text-slate-200" />
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">No hay pagos registrados</p>
                </div>
              )}
            </div>
          </div>
